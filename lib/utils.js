...
const offsetParameterIdBankA = 0;
const offsetParameterIdBankB = 64;
...
function midiNote2buttonId(note) {
  ...
  return (buttonId >= 0 && buttonId < 16) ? buttonId : null;
}
...
function parameterId2MidiController(parameterId) {
  const offsetMidiController = bank === "A" ?
    offsetParameterIdBankA : offsetParameterIdBankB;
  return parameterId + offsetMidiController;
}
...
function encoderModeController2channelId(controller) {
  const offsetMidiEncoder = bank === "A" ?
    offsetMidiEncoderBankA : offsetMidiEncoderBankB;
  const channelId = controller - offsetMidiEncoder;
  return (channelId >= 0 && channelId < 8) ? channelId : null;
}
